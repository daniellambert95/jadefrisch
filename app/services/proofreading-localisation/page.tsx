...
export default function ProofreadingLocalisationPage() {
  const { lang } = useLanguage();
  const tr = t[lang].proofreadingPage;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 items-start">
            ...
            <FadeIn delay={0.1}>
              <div className="space-y-10">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.25em] text-gold font-(family-name:--font-dm-sans) font-medium mb-6">
                    {tr.localisationListOverline}
                  </p>
                  <ul className="space-y-3">
                    {tr.localisationList.map((item, i) => (
                      <li
                        key={i}
                        className="flex items-start gap-3 font-(family-name:--font-dm-sans) font-light text-ivory/70 text-sm md:text-base"
                      >
                        <span className="w-4 h-px bg-gold mt-2.5 shrink-0" />
                        {item}
                      </li>
                ...
                <div>
                  <p className="text-[11px] uppercase tracking-[0.25em] text-gold font-(family-name:--font-dm-sans) font-medium mb-6">
                    {tr.localisationForOverline}
                  </p>
                  <ul className="space-y-3">
                    {tr.localisationFor.map((item, i) => (
                      <li
                        key={i}
                        className="flex items-start gap-3 font-(family-name:--font-dm-sans) font-light text-ivory/70 text-sm md:text-base"
                      >
